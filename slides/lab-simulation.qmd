...
- Come up with a way to empirically verify exercise 2b from the exam.
...
```{r}
w <- c(0.22, 1.96, 0.93, 1.41, 1.21, 0.73, 1.05, 0.3, 2.16, 0.86)
theta <- 3
```
...
```{r}
set.seed(602)
y <- rnorm(10, mean = theta, sd = sqrt(1/w))
y
```

Next, alternate sampling from each full conditional density (10000 times), as derived in exercise 2 from the exam. In other words, 
...
Plot histograms/density functions of the marginal distribution of each set of samples $\{\theta^{(1)}, \ldots, \theta^{(10000)}\}$. Repeat for each $w_i$. Comment on what you see. We'll get back to *why* this works in just a few classes.
